...
router.post("/", async (req, res) => {
  try {
    const { nombre, apellido, documento, nota, telefono } = req.body;
    ...
    res
      .status(500)
      .json({
        error: "Error interno del servidor al crear paciente, error 500",
      });
  }
});
...
router.get("/", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({
        error: "Error interno del servidor al listar pacientes, error 500",
      });
  }
});
